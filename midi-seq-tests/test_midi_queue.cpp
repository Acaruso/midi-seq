...
TEST(TestMidiQueue, TestMidiQueueOperations) {
    ...
    q.addEvent(createNoteOnEvent(1, 1, 100, 99));
    q.addEvent(createNoteOnEvent(1, 1, 100, 2));
    q.addEvent(createNoteOnEvent(1, 1, 100, 55));
    q.addEvent(createNoteOnEvent(1, 1, 100, 43));
    ...
    q.addEvent(createNoteOnEvent(1, 1, 100, 999));
    ...
    q.addEvent(createNoteOnEvent(1, 1, 100, 123));
...
TEST(TestMidiQueue, TestMidiQueueSideEffects) {
    ...
    q.addEvent(createNoteOnEvent(1, 1, 10, 99));
    q.addEvent(createNoteOnEvent(1, 2, 11, 2));
    q.addEvent(createNoteOnEvent(1, 3, 12, 55));
    q.addEvent(createNoteOnEvent(1, 4, 13, 43));

    q.handleEvents(43);

    EXPECT_EQ(m.getMessagesSize(), 2);
    EXPECT_EQ(m.getMessage(0), "noteOn 1 2 11");
    EXPECT_EQ(m.getMessage(1), "noteOn 1 4 13");
